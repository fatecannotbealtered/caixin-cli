...
func TestVerifyArchiveDigest(t *testing.T) {
	...
	checksums := []byte(digest + "  caixin-cli_1.0.0_linux_amd64.tar.gz\n" +
		strings.Repeat("0", 64) + "  other_asset.zip\n")

	if err := VerifyArchiveDigest(checksums, "caixin-cli_1.0.0_linux_amd64.tar.gz", archive); err != nil {
		t.Fatalf("a matching digest was rejected: %v", err)
	}

	// Tampered bytes.
	if err := VerifyArchiveDigest(checksums, "caixin-cli_1.0.0_linux_amd64.tar.gz", []byte("tampered")); err == nil {
...
func TestCertificateIdentity_PinsTheReleaseWorkflow(t *testing.T) {
	...
	if !strings.Contains(san, "owner/caixin-cli") {
