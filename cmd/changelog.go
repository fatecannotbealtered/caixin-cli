...
func parseChangelog(markdown string) []changelogEntry {
	// Release templates are kept in an HTML comment in CHANGELOG.md. Remove
	// comments before scanning headings so placeholders never become releases.
...
func (a *application) changelogCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			entries := parseChangelog(caixincli.ChangelogMarkdown)
