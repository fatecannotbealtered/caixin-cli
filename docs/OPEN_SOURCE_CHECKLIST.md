...
Run through this gate **before the first public push** of `caixin-cli`. It is a security and quality checkpoint, not documentation — every box must be ticked (or consciously waived with a note) before the repo goes public. Once public, secrets in history cannot be un-leaked.
...
- [ ] `.env`, `*.local`, credential files, and `~/.caixin-cli/` artifacts are listed in `.gitignore` and confirmed untracked (`git status --ignored`).
...
- [ ] Install blocks are copy-paste-runnable and use the real published `@fateforge/caixin-cli` / `fatecannotbealtered/caixin-cli`.
...
- [ ] If `caixin-cli` wraps a third-party product (Caixin (财新)), `NOTICE.md` carries the trademark / non-affiliation notice and `docs/COMPATIBILITY.md` lists the verified backend version matrix.
...
- [ ] `skills/caixin-cli/SKILL.md` is present; frontmatter includes `version`, `license: MIT`, `user-invocable: true`, and `metadata.requires.min_version` matching the CLI version.
...
- [ ] `skills/caixin-cli/test-prompts.json` is present, valid JSON, and covers fresh-agent read, write safety or read-only boundary, permission boundary, `_untrusted` handling, and self-update.
- [ ] A bare `update` (single command, no confirm token) syncs the whole `skills/caixin-cli/` directory or returns a `skill_sync_command` equivalent to `npx skills add fatecannotbealtered/caixin-cli -y -g`; failures/interruptions report `stage` + `current_version` + `binary_replaced` + `skill_sync_status`.
- [ ] `caixin-cli reference`, `caixin-cli context`, and `caixin-cli doctor` run and emit valid JSON envelopes — an agent can self-onboard from a clean checkout.
- [ ] `caixin-cli reference` exposes `release_readiness`, and `caixin-cli doctor` reports the matching check.
