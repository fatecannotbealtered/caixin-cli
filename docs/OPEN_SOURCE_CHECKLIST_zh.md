...
在 `caixin-cli` **首次公开推送之前**逐项走查。这是一道安全与质量关卡，不是文档 —— 仓库公开前每一项都必须勾选（或明确写明理由后豁免）。一旦公开，历史中泄露的密钥就无法收回。
...
- [ ] `.env`、`*.local`、凭据文件以及 `~/.caixin-cli/` 产物已列入 `.gitignore` 并确认未被跟踪（`git status --ignored`）。
...
- [ ] 安装块可直接复制运行，使用真实已发布的 `@fateforge/caixin-cli` / `fatecannotbealtered/caixin-cli`。
...
- [ ] 若 `caixin-cli` 包装第三方产品（Caixin (财新)），`NOTICE.md` 载明商标 / 非隶属声明，且 `docs/COMPATIBILITY.md` 列出已验证的后端版本矩阵。
...
- [ ] `skills/caixin-cli/SKILL.md` 存在；frontmatter 包含 `version`、`license: MIT`、`user-invocable: true`，且 `metadata.requires.min_version` 匹配 CLI 版本。
- [ ] `SKILL.md` 包含 `When to use`、`Do not use`、`First Step`、Agent 默认规则、JSON contract、写操作配方或明确只读边界、`STOP CHECKPOINT`、错误决策树、安全边界、自更新和评估场景。
- [ ] `skills/caixin-cli/test-prompts.json` 存在、JSON 合法，并覆盖 fresh-agent read、写操作安全或只读边界、权限边界、`_untrusted` 处理和自更新。
- [ ] 裸 `update`（单命令、无 confirm token）同步整个 `skills/caixin-cli/` 目录，或返回等价于 `npx skills add fatecannotbealtered/caixin-cli -y -g` 的 `skill_sync_command`；失败/中断报告 `stage` + `current_version` + `binary_replaced` + `skill_sync_status`。
- [ ] `caixin-cli reference`、`caixin-cli context`、`caixin-cli doctor` 可运行并输出合法的 JSON 信封 —— 代理能从干净的检出自助上手。
- [ ] `caixin-cli reference` 暴露 `release_readiness`，`caixin-cli doctor` 报告匹配的检查项。
