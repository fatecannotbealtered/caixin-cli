# Contributing to caixin-cli

*English | [中文](CONTRIBUTING_zh.md)*

Thanks for improving **caixin-cli** — Caixin (财新) CLI for AI Agents - read-only access to the homepage, channels, directories, search, topics, magazine issues, and articles the subscription entitles the user to. This document covers building, testing, and submitting changes.
...
Release standard: **Functional Contract Coverage = 100%**. Every public behavior documented in `README`, `SKILL`, reference pages, `caixin-cli reference`, `--help`, `context`, `doctor`, `changelog`, or `update` must have automated command-level tests.
...
Keep `caixin-cli reference` `release_readiness` and `caixin-cli doctor`'s `release_readiness` check honest when test evidence changes.
...
5. **SKILL** — add a `skills/caixin-cli/reference/<domain>.md` page and link it from `skills/caixin-cli/SKILL.md` (keep SKILL.md a short progressive-disclosure index).
6. **Docs** — update `README.md` / `README_zh.md` command lists and add a line under `## [Unreleased]` in `CHANGELOG.md`.

`reference` walks the command tree automatically, so new commands appear in `caixin-cli reference` without extra wiring.
