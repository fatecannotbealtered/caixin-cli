...
感谢改进 **caixin-cli** —— Caixin (财新) CLI for AI Agents - read-only access to the homepage, channels, directories, search, topics, magazine issues, and articles the subscription entitles the user to。本文档介绍如何构建、测试和提交改动。
...
发布标准：**Functional Contract Coverage = 100%**。`README`、`SKILL`、reference 页面、`caixin-cli reference`、`--help`、`context`、`doctor`、`changelog` 或 `update` 中记录的每个公开行为，都必须有自动化命令级测试。
...
当测试证据变化时，同步保持 `caixin-cli reference` 的 `release_readiness` 和 `caixin-cli doctor` 的 `release_readiness` 检查真实可信。
...
5. **SKILL** —— 新增 `skills/caixin-cli/reference/<domain>.md` 页面，并从 `skills/caixin-cli/SKILL.md` 链接过去（SKILL.md 保持为简短的渐进式披露索引）。
6. **Docs** —— 更新 `README.md` / `README_zh.md` 命令列表，并在 `CHANGELOG.md` 的 `## [Unreleased]` 下加一行。

`reference` 会自动遍历命令树，新命令无需额外接线即出现在 `caixin-cli reference` 中。
