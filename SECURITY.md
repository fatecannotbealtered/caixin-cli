...
Security policy for **caixin-cli** (@fateforge/caixin-cli) — Caixin (财新) CLI for AI Agents - read-only access to the homepage, channels, directories, search, topics, magazine issues, and articles the subscription entitles the user to.

## Supported Versions

Security fixes are applied to the **latest minor release** on the default branch. Older minors do not receive backports. Release binaries are published via GitHub Releases (`fatecannotbealtered/caixin-cli`) and the npm package `@fateforge/caixin-cli`.
...
- **GitHub private advisory** — open a draft advisory at `https://github.com/fatecannotbealtered/caixin-cli/security/advisories/new`.
...
`caixin-cli` is classified as **T1** under [`.agent/SEC-SPEC.md`](.agent/SEC-SPEC.md): every upstream command is read-only and the tool never posts or purchases, but it holds an account-level Caixin subscription session whose leak would expose the paid account, so credential handling follows the T1 baseline.
...
- **Signing key**: `article --full` needs an RSA key that Caixin ships inside its own front-end bundle. It is third-party key material, so it is **not** in this repository: it is read from `CAIXIN_SIGNING_KEY`, or sealed in the store alongside the session after being extracted once from a local browser. `context` and `doctor` report whether one is configured and which backend holds it; neither ever emits it. See [docs/FULL-TEXT.md](docs/FULL-TEXT.md).
...
- **Self-update Skill sync**: a successful bare `update` (single command, no confirm token) syncs the whole bundled `skills/caixin-cli/` directory, or returns a `skill_sync_command` equivalent to `npx skills add fatecannotbealtered/caixin-cli -y -g` when the sync did not complete.
...
Review these assumptions before integrating `caixin-cli` into automation or AI-agent workflows.
